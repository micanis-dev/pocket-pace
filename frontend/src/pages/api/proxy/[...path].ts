...
const handler: APIRoute = async ({ params, request, cookies }) => {
  ...
  try {
    const raw = cookies.get('pp_session')?.value;
    if (!raw) return new Response(JSON.stringify({ error: { message: 'ログインが必要です' } }), { status: 401, headers: { 'content-type': 'application/json' } });
    let email = '';
    try { email = JSON.parse(raw).email; } catch { /* invalid cookie */ }
    const response = await fetch(target, {
      method: request.method,
      headers: { 'content-type': request.headers.get('content-type') ?? 'application/json', 'x-pocket-pace-user': email },
