...
| 認証 | 署名付きセッション + OIDC | ローカルは簡易ログイン、本番はOIDCログイン |
| 開発環境 | Wrangler | ローカル実行とD1操作 |
...
middleware
  -> session identity / request context
```
...
- 変更時は型チェック、Lint、テストを実行する
- 外部認証やリモートD1のSecretsは使用しない
